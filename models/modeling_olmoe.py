...
class OlmoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        self.block_exp_ids = None
        self.stimu_exp_ids = None
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.block_exp_ids is not None:
            router_logits[:, self.block_exp_ids] = float("-inf")
        ...
        # if self.block_exp_ids is not None:
        #     routing_weights[:, self.block_exp_ids] = 0

        if self.stimu_exp_ids is not None:
            max_probs = routing_weights.max(dim=-1)[0]
            # import pdb; pdb.set_trace()
            routing_weights[:, self.stimu_exp_ids] = max_probs.unsqueeze(1)
        ...
        # if self.block_exp_ids is not None:
        #     mask = torch.isin(selected_experts, torch.tensor(self.block_exp_ids).to('cuda'))
        #     routing_weights[mask] = 0
        #     routing_weights /= routing_weights.sum(dim=-1, keepdim=True)
        ...
        return final_hidden_states, router_logits.view(batch_size, sequence_length, -1)
